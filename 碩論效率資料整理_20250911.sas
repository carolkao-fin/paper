...
/*Lerner index計算（用 Lerner1 + Lerner2 定義 total_revenue；含 CPI 調整）*/
/* ===========================================
...
/* ===========================================
   1) 匯入
   備註：新增 Lerner1.xlsx / Lerner2.xlsx
=========================================== */
proc import out=Lerner1
    datafile="/home/u64061874/Lerner1.xlsx" dbms=xlsx replace; getnames=yes; run;
proc import out=Lerner2
    datafile="/home/u64061874/Lerner2.xlsx" dbms=xlsx replace; getnames=yes; run;
...
/* 將可能為字串的金額欄位安全轉成數值（含全形→半形、去逗號、保留負號與小數點）*/
%macro numize(srcvar, dstvar);
  length _raw _half _keep $32767;
  if vtype(&srcvar)='N' then &dstvar = &srcvar;
  else do;
    _raw  = vvaluex("&srcvar");
    /* 轉半形數字與小數點、保留 '-' ；去空白/逗號 */
    _half = translate(_raw, '0123456789.-', '０１２３４５６７８９．－');
    /* 若原始用括號表負值，可選擇性處理： */
    if findc(_half, '()') then do;
      _keep = '-'||compress(translate(_half,'0123456789.-','() ,'), ' ');
    end;
    else _keep = compress(_half, ' ,');
    if _keep in ('', '-', '.') then &dstvar = .;
    else &dstvar = inputn(_keep, 'best32.');
  end;
%mend;
...
data pe2_std;
  ...
  %std_company_id(companyID_raw, companyID);
  %make_years(year_raw, year, year_new);       /* year: 西元；year_new: 索引年 */
...
/* ===========================================
   5) Lerner 收入合計（替換版）：
      total_revenue = Lerner1.BR + (Lerner2.net_income + operate_cost + income_outside)
...
data Lerner1_sum;
  length companyID $200;
  set Lerner1(rename=(companyID=companyID_raw year=year_raw));
  %std_company_id(companyID_raw, companyID);
  %make_years(year_raw, year_num, year_new);
  ...
  keep companyID year_num year_new year revenue1;
run;

/* Lerner2：標準化 + 取 net_income + operate_cost + income_outside */
data Lerner2_sum;
  length companyID $200;
  set Lerner2(rename=(companyID=companyID_raw year=year_raw));
  %std_company_id(companyID_raw, companyID);
  %make_years(year_raw, year_num, year_new);
  year = coalesce(year_num, year_new + 2002);

  /* 轉數值並加總 */
  %numize(net_income,     net_income_n);
  %numize(operate_cost,   operate_cost_n);
  %numize(income_outside, income_outside_n);

  revenue2 = sum(net_income_n, operate_cost_n, income_outside_n);
  keep companyID year_num year_new year revenue2;
run;

/* 合併 Lerner1 + Lerner2 → total_revenue */
proc sql;
  create table Lerner_revenue as
  select coalesce(a.companyID, b.companyID) as companyID length=200,
         coalesce(a.year, b.year)           as year,
         coalesce(a.year_num, b.year_num)   as year_num,
         coalesce(a.year_new, b.year_new)   as year_new,
         a.revenue1,
         b.revenue2,
         sum(a.revenue1, b.revenue2)        as total_revenue
  from Lerner1_sum a
  full join Lerner2_sum b
...
proc sql;
  ...
  select a.companyID,
         ...
         case when b.index > 0 then (a.total_revenue / b.index) * 100
              else . end as total_revenue_adj      /* ← 實質收入（給下游用） */
...
/* ===========================================
   7) 以 companyID 對齊；年份規則：pe4.year = pe2.year_new
=========================================== */
proc sql;
...
data final_LI_with_name2;
    ...
    sample_num = input(sample, best32.);
run;
...
proc sql;
  select year,
         ...
         sum(case when LI<0 then 1 else 0 end)*100.0 / count(*) as pct_neg
  from final_LI_export
  where LI is not null
  group by year;
quit;

/*2003–2023 全樣本的 LI 為負比率*/
proc sql;
  select count(*) as total_n,
         sum(case when LI<0 then 1 else 0 end) as n_neg,
         calculated n_neg*100.0 / calculated total_n as pct_neg
  from final_LI_export
  where LI is not null;
quit;
...
/* ===========================================
   計算每年 LI > 1 的比率
   資料集: paper_LI_result_final2
   =========================================== */
proc sql;
  create table LI_gt1_stats as
  select year,
         count(*) as total_n,   /* 該年度樣本總數 */
         sum(case when LI>1 then 1 else 0 end) as n_gt1,  /* LI > 1 的數量 */
         calculated n_gt1*100.0 / calculated total_n as pct_gt1  /* 比率 (%) */
  from paper_LI_result_final2
  where LI is not null
  group by year
  order by year;
quit;
...
/* 檢視結果 */
proc print data=LI_gt1_stats noobs; run;

/* ===========================================
   列出 LI > 1 的公司與年份
   =========================================== */
proc sql;
  create table LI_gt1_detail as
  select companyID,
         companyName,
         year,
         LI
...
/* 顯示前 50 筆檢查 */
proc print data=LI_gt1_detail (obs=50) noobs; run;

/* ===========================================
   計算 LI 不在 0~1 區間的整體比率
   =========================================== */
proc sql;
  select count(*) as total_n,                           /* 總樣本數 */
         sum(case when LI<0 or LI>1 then 1 else 0 end) as n_outlier,  /* 異常數量 */
         calculated n_outlier*100.0 / calculated total_n as pct_outlier /* 比率 (%) */
  from paper_LI_result_final
  where LI is not null;
quit;

/* ===========================================
   拆分 LI < 0 與 LI > 1 的數量與比率
   =========================================== */
proc sql;
  select count(*) as total_n,
         sum(case when LI < 0 then 1 else 0 end) as n_lt0,
         sum(case when LI > 1 then 1 else 0 end) as n_gt1,
         calculated n_lt0*100.0 / calculated total_n as pct_lt0,
         calculated n_gt1*100.0 / calculated total_n as pct_gt1
  from paper_LI_result_final
  where LI is not null;
quit;
