...
/* HHI：如為字串先轉數值；再去重（companyID-year 唯一） */
data _hhi_prep;
    length companyID $16;
    set paper_HHI_final(keep=companyID year hhi);
    companyID = strip(companyID);
    /* 若 hhi 是字串（常見：含逗號），轉為數值 */
    if vtype(hhi)='C' then do;
        hhi_num = input(compress(hhi, ', '), best32.);
        hhi = hhi_num;
        drop hhi_num;
    end;
run;
...
/* 只留指定欄位（不含 ln_UR），刪除 ln_BR / MSO / BS 缺失 */
